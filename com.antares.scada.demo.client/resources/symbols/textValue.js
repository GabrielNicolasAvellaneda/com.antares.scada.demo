/*******************************************************************************
 * Copyright (c) 2013 IBH SYSTEMS GmbH and others. All rights reserved. This
...
function initTextValue() {
	...
	controller.getElement("label")
			.setToolTip(controller.getProperty("tooltip"));

	controller.registerItem("A", controller.getProperty("item"), controller
...
function updateValue(itemName) {
	...
	if (value == null) {
		...
		// set text
		controller.getElement("label").setText(
				java.lang.String.format(format, [ value ]));
	}
}

function onClick() {
	openItemDetails(controller.getProperty("connection"), controller
			.getProperty("item"));
}
